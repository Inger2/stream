...
public class Main {
    ...
    public static int findBestDriftParkingId(List<Parking> parkings, List<Car> cars) {
        Comparator<Parking> comparator = Comparator
                .comparingDouble((Parking x) -> {
                    ParkingService parkingService = new ParkingService(x);
                    return parkingService.bestParkingPercentage(cars);
                }).thenComparingInt(Parking::getCapacity);
